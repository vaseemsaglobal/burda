report 50174 "Tax Invoice (SG)"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'Layouts/50174_Tax Invoice (SG).rdl';
    Caption = 'Tax Invoice (SG)';
    ...
    dataset
    {
        dataitem("Sales Invoice Header";"Sales Invoice Header")
        {
            ...
            dataitem(CopyLoop;"Integer")
            {
                ...
                dataitem(PageLoop;"Integer")
                {
                    ...
                    dataitem("Sales Invoice Line";"Sales Invoice Line")
                    {
                        ...
                        column(Vatper;VATAmountLine."VAT %")
                        {
                        }
                        ...
                        trigger OnPreDataItem()begin
                            /*GenJnlLine3.COPY("Gen. Journal Line");
                                                    ...
                                                    IF (GenJnlLine3."Document No." = GenJnlLine2."Document No.") THEN
                                                      CurrReport.BREAK;  */
                            if CurrReport.PageNo = 1 then LineSpace:=15
                            else
                                LineSpace:=25;
                        end;
                    }
                }
                trigger OnAfterGetRecord()var salesinvoiceLine: Record "Sales Invoice Line";
                begin
                    ...
                    if PrintSummary then begin
                        ...
                        IF salesinvoiceLine.FindSet()then repeat TotalLineDiscountAmountprint+=salesinvoiceLine."Line Discount Amount";
                    ...
                    if "Sales Invoice Header"."Currency Code" = '' then begin
                        ...
                        currencyDesc:=GLSetup."Local Currency Description" end
                    else
                    begin
            ...
            begin
                ...
                if CommentText = '' then Remark2:="Sales Invoice Header"."Remark 2"
                else
                    Remark2:=CommentText;
            end;
            trigger OnPreDataItem()begin
    ...
    local procedure FormatDocumentFields(SalesInvoiceHeader: Record "Sales Invoice Header")begin
        ...
        if SalesPurchPerson.Get(SalesInvoiceHeader."Salesperson Code")then SalesPersonText:=SalesPurchPerson.Name
        else
            SalesPersonText:='';
    end;
    local procedure FormatAddressFields(SalesInvoiceHeader: Record "Sales Invoice Header")begin
        ...
        if(SalesInvoiceHeader."Bill-to Name (Thai)" = '')then CustAddr[1]:=SalesInvoiceHeader."Bill-to Name"
        else
    ...
    begin
        ...
        x:=STRPOS(AmountText, '.');
        CASE TRUE OF x = 0: AmountText:=AmountText + '.00';
        x = STRLEN(AmountText) - 1: AmountText:=AmountText + '0';
        x > STRLEN(AmountText) - 2: AmountText:=COPYSTR(AmountText, 1, x + 2);
        end;
        l:=STRLEN(AmountText);
        REPEAT dflag:=FALSE;
            ...
            l:=l - 1;
        UNTIL l = 3;
        IF COPYSTR(AmountText, STRLEN(AmountText) - 2, 3) = '.00' THEN WHTAmtThaiText:=WHTAmtThaiText + 'Š­‡†Ò—‰'
        ELSE
        begin
    ...
    pos: Integer;
    dflag: Boolean)fdigitCount: text[100]var fdigit: Text[30];
    fcount: Text[30];
    begin
        CASE adigit OF '1': begin
            IF(pos IN[1, 9, 17]) AND dflag THEN fdigit:='ÊÏ„'
            ELSE IF pos IN[2, 10, 18]THEN fdigit:=''
                ELSE
                    fdigit:='›‰´Ðº';
        end;
        '2': begin
            IF pos IN[2, 10, 18]THEN fdigit:='’³Ð'
            ELSE
                fdigit:='šº';
        end;
        '3': fdigit:='š­‘';
        ...
        IF(adigit <> '0') AND (adigit <> '-')THEN begin
            CASE pos OF 2, 10, 18: fcount:='š¯Š';
            3, 11, 19: fcount:='“Ò’';
            5, 13, 21: fcount:='Ž©‰';
            6, 14, 22: fcount:='›‘¸Ð‰';
            7, 15, 23: fcount:='ßš‰';
    ...
    local procedure SetLanguageCaption()var LanguageCaption: Record "Language Caption";
    begin
        LanguageCaption.Reset();
        LanguageCaption.SetRange("Report ID", 50174);
        if LanguageCaption.FindSet()then begin
            repeat case LanguageCaption."Caption Code" of 'TaxID': begin
    ...
    procedure WorkDescriptionFromDoc(pWorkDescription: Text)begin
