Report 50506 "Petty Cash Voucher"
{
    ApplicationArea = all;
    UsageCategory = ReportsAndAnalysis;
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/PettyCashVoucher.rdl';
    ...
    dataset
    {
        dataitem("Petty Cash Header"; "Petty Cash Header")
        {
            ...
            column(Description; "Petty Cash Header"."Invoice Description")
            {
            }
            column(xCash; xCash)
            {
            ...
            dataitem("Petty Cash Line"; "Petty Cash Line")
            {
                DataItemTableView = sorting("Document No.", "Line No.") WHERE(Type = FILTER(<> " "));
                DataItemLink = "Document no." = FIELD("No.");

                column(EntryNo_VendLedgerEntrybuffer2; "Petty Cash Line"."Line No.")
                {
                }
                column(Description_ExternalDocumentNo; "Petty Cash Line".Description + '   #' + "Petty Cash Line"."External Document No.")
                {
                }
                column(VendLedgerEntrybuffe2_AmounttoApplyCreditVATAmtTmp; "Petty Cash Line"."VAT Base Amount")
                {
                }
                column(CreditVATAmtTmp; "Petty Cash Line"."VAT Amount")
                {
                }
                column(VendLedgerEntrybuffe2_AmounttoApply; "Petty Cash Line"."VAT Base Amount" + "Petty Cash Line"."VAT Amount")
                {
                }
                ...
                begin
                    ...
                    REPEAT
                        i := i + 1;
                        IF ((AccCode[i] = AccountNo) AND (LineDescription[i] = Description) AND (Dimension1Code[i] = "Shortcut Dimension 1 Code") AND (Dimension2Code[i] = "Shortcut Dimension 2 Code") AND (Dimension3Code[i] = ShortcutDim3Code) AND (Dimension4Code[i] = ShortcutDim4Code) AND (Dimension5Code[i] = ShortcutDim5Code)) OR (AccCode[i] = '') THEN MatchAcc := TRUE;
                    UNTIL (i = 200) OR MatchAcc;
                    AccCode[i] := AccountNo;
                    AccName[i] := "Petty Cash Line".Description;
                    ...
                    IF NOT "Skip WHT" THEN BEGIN
                        ...
                        IF WHTPostingSetup."WHT %" <> 0 THEN BEGIN
                            ...
                            REPEAT
                                i := i + 1;
                                IF (WHTCode[i] = AccountNo) OR (WHTCode[i] = '') THEN MatchAcc := TRUE;
                            UNTIL (i = 200) OR MatchAcc;
                            WHTCode[i] := AccountNo;
            ...
            dataitem(Detail; "Integer")
            {
                ...
                begin
                    ...
                    REPEAT
                        i := i + 1;
                        IF ((AccCode[i] = VendPostingGrp."Payables Account") AND (Dimension1Code[i] = "Petty Cash Header"."Shortcut Dimension 1 Code") AND (Dimension2Code[i] = "Petty Cash Header"."Shortcut Dimension 2 Code") AND (Dimension3Code[i] = ShortcutDim3Code) AND (Dimension4Code[i] = ShortcutDim4Code) AND (Dimension5Code[i] = ShortcutDim5Code)) OR (AccCode[i] = '') THEN MatchAcc := TRUE UNTIL (i = 200) OR MatchAcc;
                    AccCode[i] := (VendPostingGrp."Payables Account");
                    //AccName[i] := GLAcc.Name; //Ravi
                    AccName[i] := "Petty Cash Header"."Invoice Description";
                    //Ravi
                ...
                begin
                    ...
                    CheckReport.FormatNoText(TextAmount, Abs(DebitTotalLCY), "Petty Cash Header"."Currency Code");
                    if TextAmount[1] <> '' then
                        TextAmount[1] := '(' + TextAmount[1] + ' ' + TtoalCurrCode + ')';
                    if CrAmount[Number] <> 0 then // CurrReport.Skip;
                        CreditTotalLCY := CreditTotalLCY + CrAmount[Number] + ROUND("Petty Cash Line"."VAT Base Amount (LCY)" * WHTPostingSetup."WHT %" / 100);
                    ;

                    CheckReport.InitTextVariable;
                    CheckReport.FormatNoText(TextAmount, Abs(CreditTotalLCY), "Petty Cash Header"."Currency Code");
                    if TextAmount[1] <> '' then
                        TextAmount[1] := '(' + TextAmount[1] + ' ' + TtoalCurrCode + ')';
                    ///    ROUND("Petty Cash Line"."VAT Base Amount (LCY)" * WHTPostingSetup."WHT %" / 100);
                    /// 
                    ///
                end;
            }
            ...
            dataitem(Description1; "Integer")
            {
                ...
                column(CreditVATNameNumber; CreditWHTAmt) // CreditVATName[Number])
                {
                }
                column(CreditVATAmtNumber; CreditWHTAmt) // CreditVATAmt[Number])
                {
                }
                trigger OnAfterGetRecord()
                begin
            ...
            dataitem("Line Loop"; Integer)
            {
                ...
                column(CreditAccNameNumber; CreditWHTAmt) // CreditAccName[Number])
                {
                }
                ...
                trigger OnAfterGetRecord()
                begin
                    IF CntLine + Line + Number > 21 THEN CurrReport.BREAK;
                end;
            ...
            begin
                ...
                CLEAR(WHTCrAmount);
                Clear(PayeeName);
                if "Petty Cash Header"."Name (Thai)" <> '' then begin
                    PayeeName := "Petty Cash Header"."Petty Cash Vendor No." + ' ' + "Petty Cash Header"."Name (Thai)";

                end else begin
                    PayeeName := "Petty Cash Header"."Petty Cash Vendor No." + ' ' + "Petty Cash Header".Name;
                    VendName := "Petty Cash Header".Name;
                end;

                if "Petty Cash Header"."Name 2" <> '' then
                    VendName := "Petty Cash Header"."Name 2"
                else
                    VendName := "Petty Cash Header".Name;
                ...
                LanguageCaption.SetRange("Report ID", 50506);
            ...
            trigger OnPreDataItem()
            begin
                //Ravi    CurrReport.CREATETOTALS(DrAmount, CrAmount);
            end;
        }
    }
    requestpage
